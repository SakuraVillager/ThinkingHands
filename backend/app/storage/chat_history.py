...
def get_session_info(session_id: str) -> Optional[Dict[str, str]]:
    """
    获取指定会话的元数据信息。
    Args:
        session_id: 会话ID
    Returns:
        Dict 或 None: 包含会话信息的字典，如果会话不存在则返回 None
    """
    sql = '''
        SELECT 
            cs.session_id,
